<template>
  <div class="overflow-hidden">
        ...
        >
          <div class="flex items-start justify-between gap-3">
            <div class="flex-1">
              <div class="flex items-start gap-3">
                <div class="flex-1">
                  <p class="text-gray-700 dark:text-gray-200 font-medium text-lg">{{ code.code }}</p>
                  <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">{{ code.description }}</p>
                </div>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <Badge
                :color="code.status === 'active' ? 'success' : code.status === 'expired' ? 'error' : code.status === 'upcoming' ? 'warning' : 'secondary'"
                size="sm"
              >
                {{ t(`discountCodes.${code.status}`) }}
              </Badge>
            </div>
          </div>
          ...
          <div class="mt-3 text-sm text-gray-600 dark:text-gray-400">
            <div class="flex flex-col gap-1">
              <div class="flex items-center gap-2">
                <Badge :color="code.type === 'percentage' ? 'info' : 'success'" size="sm">
                  {{ code.type === 'percentage' ? t('discountCodes.percentage') : t('discountCodes.fixed') }}
                </Badge>
                <span class="font-medium text-gray-700 dark:text-gray-200">
                  <span v-if="code.type === 'percentage'">{{ code.value }}%</span>
                  <span v-else>{{ code.value }} {{ t('common.currency') }}</span>
                </span>
              </div>
              <div class="mt-2">
                <strong class="text-gray-700 dark:text-gray-200">{{ t('discountCodes.validity') }}:</strong>
                <div class="text-xs mt-1">
                  <div>{{ code.start_date }}</div>
                  <div>{{ code.end_date }}</div>
                </div>
              </div>
              <div class="mt-1">
                <strong class="text-gray-700 dark:text-gray-200">{{ t('discountCodes.usage') }}:</strong>
                <span class="ml-1">{{ code.usage_count }} / {{ code.usage_limit || '∞' }}</span>
                <span class="text-xs text-gray-500 ml-1">({{ code.usage_percentage }}%)</span>
              </div>
            </div>
          </div>
          ...
          <div class="mt-4 flex items-center justify-end">
            <div class="flex items-center gap-2">
              ...
              <Tooltip :text="t('messages.notAuthorized')" :show="!canDelete">
                <button
                  :disabled="!canDelete"
                  @click="handleDeleteClick(code.id)"
    ...
    <DangerAlert
      ...
      :message="t('discountCodes.confirmDelete')"
...
const canCreate = computed(() => hasAnyPermission(['discount-codes.create', 'discount-codes.store', 'discount-codes.add']))
const canView   = computed(() => hasAnyPermission(['discount-codes.view', 'discount-codes.show', 'discount-codes.read']))
const canEdit   = computed(() => hasAnyPermission(['discount-codes.update', 'discount-codes.edit']))
const canDelete = computed(() => hasAnyPermission(['discount-codes.delete', 'discount-codes.destroy']))

const { t } = useI18n()
const { success, error } = useNotifications()

const props = defineProps({ codes: Object })

const search         = ref('')
const currentPage    = ref(props.codes?.current_page ?? 1)
const perPage        = ref(props.codes?.per_page ?? 9)

function goToCreate()      { router.visit(route('admin.discount-codes.create')) }
function goToView(id)      { router.visit(route('admin.discount-codes.show', id)) }
function goToEdit(id)      { router.visit(route('admin.discount-codes.edit', id)) }
...
const codeToDeleteId = ref(null)

function openDeleteModal(id)  { codeToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; codeToDeleteId.value = null }

function confirmDelete() {
  if (codeToDeleteId.value) {
    router.delete(route('admin.discount-codes.destroy', codeToDeleteId.value), {
      onSuccess: () => { success(t('discountCodes.deleteSuccess')); closeDeleteModal() },
      onError:   () => { error(t('discountCodes.deleteFailed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}
...
const totalEntries = computed(() => props.codes?.total || filteredData.value.length)
const startEntry   = computed(() => props.codes?.from  || 1)
const endEntry     = computed(() => props.codes?.to    || filteredData.value.length)
const totalPages   = computed(() => props.codes?.last_page || 1)
...
watch(() => props.codes?.current_page, (val) => {
  currentPage.value = typeof val === 'number' ? val : 1
})
watch(() => props.codes?.per_page, (val) => {
