<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
      <Link :href="route('admin.bookings.index')" class="shadow-theme-xs inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 ring-1 ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">{{ t('buttons.backToList') }}</Link>
      <button @click="create" :disabled="form.processing" class="bg-brand-500 shadow-theme-xs hover:bg-brand-600 inline-flex items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition disabled:opacity-50">
        <span v-if="form.processing">{{ t('buttons.creating') }}...</span>
...
/**
 * Normalize ISO datetime strings coming from the backend into
 * the formats expected by the inputs:
 *  - date: YYYY-MM-DD
...
const normalizeDateTimeInputs = () => {
  // Normalize start_time if backend sent a full ISO datetime
  ...
  // Normalize date if it's an ISO datetime
...
// In case the parent injects ISO values after mount, watch and normalize
watch(() => form.start_time, (val) => {
  if (val && typeof val === 'string' && val.includes('T')) {
    form.start_time = isoToLocalTime(val)
  }
})

watch(() => form.date, (val) => {
  if (val && typeof val === 'string' && val.includes('T')) {
    form.date = isoToLocalDate(val)
  }
})
...
const formatTime = (val) => {
  ...
  // If it's already in H:i:s or H:i, normalize to H:i
  if (val.includes(':')) {
...
const create = () => {
  ...
  form.post(route('admin.bookings.store'), {
    onSuccess: () => {
      success(t('booking.booking_created_successfully'))
    },
    onError: (errors) => {
      error(t('booking.booking_creation_failed'))
    },
    preserveScroll: true,
  })
}
