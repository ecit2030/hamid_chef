<template>
  <div class="overflow-hidden">
        ...
        >
              ...
              <div class="flex items-start gap-3">
                <div class="flex-1">
                  <p class="text-gray-700 dark:text-gray-200 font-medium text-lg">
                    {{ locale === 'ar' ? term.title_ar : term.title_en }}
                  </p>
                  <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
                    {{ t('terms_and_conditions.version') }}: {{ term.version }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="mt-3 text-sm text-gray-600 dark:text-gray-400">
            <div class="flex flex-col gap-1">
              <div>
                <strong class="text-gray-700 dark:text-gray-200">{{ t('terms_and_conditions.effective_date') }}:</strong>
                <span class="ml-1">{{ formatDate(term.effective_date) }}</span>
              </div>
              <div class="mt-2">
                <strong class="text-gray-700 dark:text-gray-200">{{ t('terms_and_conditions.title_ar') }}:</strong>
                <p class="text-xs mt-1 line-clamp-2">{{ term.title_ar }}</p>
              </div>
              <div class="mt-1">
                <strong class="text-gray-700 dark:text-gray-200">{{ t('terms_and_conditions.title_en') }}:</strong>
                <p class="text-xs mt-1 line-clamp-2">{{ term.title_en }}</p>
              </div>
            </div>
          </div>
    ...
    <DangerAlert
      ...
      :message="t('terms_and_conditions.confirm_delete')"
...
const canCreate = computed(() => hasAnyPermission(['terms-and-conditions.create', 'terms-and-conditions.store', 'terms-and-conditions.add']))
const canEdit   = computed(() => hasAnyPermission(['terms-and-conditions.update', 'terms-and-conditions.edit']))
const canDelete = computed(() => hasAnyPermission(['terms-and-conditions.delete', 'terms-and-conditions.destroy']))
...
const perPage        = ref(props.terms?.per_page ?? 9)

function goToCreate()      { router.visit(route('admin.terms-and-conditions.create')) }
function goToEdit(id)      { router.visit(route('admin.terms-and-conditions.edit', id)) }
...
function openDeleteModal(id)  { termToDeleteId.value = id; isDeleteModalOpen.value = true }
function handleDeleteClick(id){ if (!canDelete.value) return; openDeleteModal(id) }
function closeDeleteModal()   { isDeleteModalOpen.value = false; termToDeleteId.value = null }

function confirmDelete() {
  if (termToDeleteId.value) {
    router.delete(route('admin.terms-and-conditions.destroy', termToDeleteId.value), {
      onSuccess: () => { success(t('terms_and_conditions.delete_success')); closeDeleteModal() },
      onError:   () => { error(t('terms_and_conditions.delete_failed'));       closeDeleteModal() },
      preserveScroll: true,
    })
  }
}
...
function toggleStatus(term) {
  const wasActive = term.is_active
  term.is_active = !wasActive
  const url = wasActive
    ? route('admin.terms-and-conditions.deactivate', { id: term.id })
    : route('admin.terms-and-conditions.activate',   { id: term.id })
  router.patch(url, {}, {
    preserveState: true,
    preserveScroll: true,
    onError: () => { term.is_active = wasActive },
  })
}
...
const filteredData = computed(() => {
  ...
  return (props.terms?.data || [])
    .filter((d) => {
      const titleAr = d.title_ar?.toLowerCase() || ''
      const titleEn = d.title_en?.toLowerCase() || ''
      const version = d.version?.toLowerCase() || ''
      return titleAr.includes(searchLower) || titleEn.includes(searchLower) || version.includes(searchLower)
    })
})
...
const startEntry   = computed(() => props.terms?.from  || 1)
