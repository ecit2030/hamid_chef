<template>
  <section class="relative py-24 overflow-hidden bg-primary dark:bg-gray-900">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
      ...
      <!-- Chefs Grid -->
      <div v-if="chefs.length > 0" class="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div
        ...
        >
          <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden hover:border-secondary transition-colors duration-300">
            <!-- Image -->
            ...
            <div class="p-5 text-center">
              <h3 class="text-lg font-black text-primary dark:text-white mb-1">{{ chef.name }}</h3>
      ...
      <div v-else class="text-center py-12">
        <div class="w-24 h-24 mx-auto mb-6 bg-secondary/20 rounded-full flex items-center justify-center">
          <span class="text-5xl">👨‍🍳</span>
        </div>
        <p class="text-white/80 text-lg">
          {{ currentLang === 'ar' ? 'سيتم عرض أفضل الطهاة قريباً' : 'Top chefs coming soon' }}
        </p>
      </div>
      ...
      <div v-if="section?.additional_data?.show_cta" class="text-center mt-10">
        <a
          ...
          class="inline-flex items-center gap-3 px-8 py-4 bg-secondary text-primary font-black rounded-xl hover:bg-white transition-colors duration-300"
        >
