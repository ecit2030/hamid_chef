<template>
  <div class="overflow-hidden">
            ...
            <div class="flex items-center gap-2">
              ...
              >
                <svg class="fill-current" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10.5 18.375C14.8492 18.375 18.375 14.8492 18.375 10.5C18.375 6.15076 14.8492 2.625 10.5 2.625C6.15076 2.625 2.625 6.15076 2.625 10.5C2.625 14.8492 6.15076 18.375 10.5 18.375ZM14.7803 8.28033C15.0732 7.98744 15.0732 7.51256 14.7803 7.21967C14.4874 6.92678 14.0126 6.92678 13.7197 7.21967L9.25 11.6893L7.28033 9.71967C6.98744 9.42678 6.51256 9.42678 6.21967 9.71967C5.92678 10.0126 5.92678 10.4874 6.21967 10.7803L8.71967 13.2803C9.01256 13.5732 9.48744 13.5732 9.78033 13.2803L14.7803 8.28033Z" />
                </svg>
              </button>
              ...
              >
                <svg class="fill-current" width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd" d="M10.5 18.375C14.8492 18.375 18.375 14.8492 18.375 10.5C18.375 6.15076 14.8492 2.625 10.5 2.625C6.15076 2.625 2.625 6.15076 2.625 10.5C2.625 14.8492 6.15076 18.375 10.5 18.375ZM8.21967 7.21967C7.92678 7.51256 7.92678 7.98744 8.21967 8.28033L9.43934 9.5L8.21967 10.7197C7.92678 11.0126 7.92678 11.4874 8.21967 11.7803C8.51256 12.0732 8.98744 12.0732 9.28033 11.7803L10.5 10.5607L11.7197 11.7803C12.0126 12.0732 12.4874 12.0732 12.7803 11.7803C13.0732 11.4874 13.0732 11.0126 12.7803 10.7197L11.5607 9.5L12.7803 8.28033C13.0732 7.98744 13.0732 7.51256 12.7803 7.21967C12.4874 6.92678 12.0126 6.92678 11.7197 7.21967L10.5 8.43934L9.28033 7.21967C8.98744 6.92678 8.51256 6.92678 8.21967 7.21967Z" />
                </svg>
              </button>
    ...
    <!-- Map View -->
    <div v-show="activeTab === 'map'" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] overflow-hidden">
      <div class="p-4 border-b border-gray-200 dark:border-gray-800">
        <h3 class="text-lg font-medium text-gray-800 dark:text-white">{{ t('booking.bookings_map') }}</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ t('booking.map_legend') }}</p>
        <!-- Legend -->
        <div class="flex flex-wrap gap-4 mt-3">
          <div class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-red-500"></span>
            <span class="text-xs text-gray-600 dark:text-gray-400">{{ t('booking.urgent_today') }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-yellow-500"></span>
            <span class="text-xs text-gray-600 dark:text-gray-400">{{ t('booking.soon_3_days') }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-green-500"></span>
            <span class="text-xs text-gray-600 dark:text-gray-400">{{ t('booking.later') }}</span>
          </div>
        </div>
      </div>
      ...
      <div class="relative" style="height: 500px;">
        ...
        <div v-if="!bookingsWithCoordinates.length" class="absolute inset-0 flex items-center justify-center bg-gray-50 dark:bg-gray-900">
          <div class="text-center">
            <svg class="w-16 h-16 mx-auto text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            <p class="mt-4 text-gray-500 dark:text-gray-400">{{ t('booking.no_bookings_with_location') }}</p>
          </div>
        </div>
      </div>

      <!-- Bookings List under map -->
      <div v-if="bookingsWithCoordinates.length" class="p-4 border-t border-gray-200 dark:border-gray-800">
    ...
    <DangerAlert 
      ...
      :title="t('booking.reject_booking')" 
      :message="t('booking.reject_booking_confirmation')" 
...
function handleAcceptClick(id) {
  router.patch(route('chef.bookings.accept', id), {}, {
...
function handleCompleteClick(id) {
  router.patch(route('chef.bookings.complete', id), {}, {
...
const bookingsWithCoordinates = computed(() => {
  return (props.bookings?.data || []).filter(b => b.address?.lat && b.address?.lang)
})
...
const initMap = () => {
  ...
  bookingsWithCoordinates.value.forEach(booking => {
    bounds.extend({ lat: parseFloat(booking.address.lat), lng: parseFloat(booking.address.lang) })
  })
  ...
  bookingsWithCoordinates.value.forEach(booking => {
    const marker = new google.maps.Marker({
      position: { lat: parseFloat(booking.address.lat), lng: parseFloat(booking.address.lang) },
      map: map,
      icon: {
