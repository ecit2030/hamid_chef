<template>
    <AdminLayout>
        <PageBreadcrumb :pageTitle="$t('terms_and_conditions.edit')" />
...
const form = useForm({
    title_ar: props.terms.title_ar,
    title_en: props.terms.title_en,
    content_ar: props.terms.content_ar,
    content_en: props.terms.content_en,
    version: props.terms.version,
    effective_date: props.terms.effective_date
        ? props.terms.effective_date.split("T")[0]
        : "",
    is_active: props.terms.is_active,
});

const submit = () => {
    form.put(route("admin.terms-and-conditions.update", props.terms.id));
};
</script>
